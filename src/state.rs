		...
		match *event {
			...
			Event::UserServerSettingsUpdate(ref settings) => {
				if let Some(server_settings) = self.server_settings.as_mut() {
					server_settings
						...
						.map(|srv| { srv.clone_from(settings); });
				}
			}
			Event::VoiceStateUpdate(None, ref state) => {
				if let Some(channel) = state.channel_id {
					...
					if let Some(call) = self.calls.get_mut(&channel) {
						if let Some(grp_state) = call.voice_states
						       .iter_mut()
				...
				self.servers
					...
					.map(|srv| {
						if !state.channel_id.is_some() {
							...
							if let Some(srv_state) = srv.voice_states
							       .iter_mut()
							       .find(|u| u.user_id == state.user_id) {
			...
			Event::ServerMembersChunk(server_id, ref members) => {
				self.servers
					.iter_mut()
					.find(|s| s.id == server_id)
					.map(|srv| { srv.members.extend_from_slice(members); });
			}
			Event::ServerSync {
				server_id,
				large,
			...
			Event::ServerRoleCreate(ref server_id, ref role) => {
				self.servers
					.iter_mut()
					.find(|s| s.id == *server_id)
					.map(|srv| { srv.roles.push(role.clone()); });
			}
				...
				self.servers
					...
					.map(|srv| {
						     srv.roles
						         .iter_mut()
						         .find(|r| r.id == role.id)
						         .map(|srv_role| { srv_role.clone_from(role); });
						    });
			}
			Event::ServerRoleDelete(ref server_id, ref role_id) => {
				self.servers
					.iter_mut()
					.find(|s| s.id == *server_id)
					.map(|srv| { srv.roles.retain(|r| r.id != *role_id); });
			}
				...
				match *channel {
					...
					Channel::Public(ref channel) => {
						self.servers
							...
							.map(|srv| { srv.channels.push(channel.clone()); });
					}
				}
			}
						...
						match self.groups.entry(group.channel_id) {
							...
							Entry::Occupied(mut e) => {
								...
								if group.recipients.is_empty() {
									...
									let recipients = ::std::mem::replace(&mut dest.recipients,
									                                     Vec::new());
				...
				match *channel {
					...
					Channel::Public(ref channel) => {
						self.servers
							.iter_mut()
							.find(|s| s.id == channel.server_id)
							.map(|srv| { srv.channels.retain(|c| c.id != channel.id); });
					}
				}
			}
